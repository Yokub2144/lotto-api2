...
public async Task<IActionResult> ClearAllData()
{
    ...
    // 1) ลบ Orders (FK -> Lottery, User)
    _db.Orders.RemoveRange(_db.Orders);
    await _db.SaveChangesAsync();

    // 2) ลบ Reward (FK -> Lottery)
    _db.Reward.RemoveRange(_db.Reward);
    await _db.SaveChangesAsync();

    // 3) ลบ Wallet ที่ไม่ใช่ของ admin (FK -> User)
    var walletsToDelete = await _db.Wallet
        .Where(w => !_db.User.Any(u => u.uid == w.uid && u.role == "admin"))
        .ToListAsync();
    _db.Wallet.RemoveRange(walletsToDelete);
    await _db.SaveChangesAsync();

    // 4) ลบ Lottery (ตอนนี้ไม่มี Reward อ้างแล้ว)
    _db.Lottery.RemoveRange(_db.Lottery);
    await _db.SaveChangesAsync();

    // 5) ลบ User ที่ไม่ใช่ admin (Wallet ถูกลบไปแล้ว)
    var usersToDelete = await _db.User.Where(u => u.role != "admin").ToListAsync();
    _db.User.RemoveRange(usersToDelete);
    await _db.SaveChangesAsync();

    await tx.CommitAsync();

    return Ok(new { message = "ลบข้อมูลเรียบร้อย (ยกเว้น admin)" });
}
